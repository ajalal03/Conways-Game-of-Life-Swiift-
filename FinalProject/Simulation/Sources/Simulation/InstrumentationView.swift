        ...
        WithViewStore(store) { viewStore in
            HStack {
                ...
                ThemedButton(text: "Step") {
                    viewStore.send(.stepGrid, animation: .interpolatingSpring(stiffness: 100, damping: 30))
                    viewStore.send(.animategrid)
                    viewStore.send(.unanimate)
                }
                Spacer()
                ThemedButton(text: "Empty") { viewStore.send(.resetGridToEmpty, animation: .interpolatingSpring(stiffness: 100, damping: 30))
                    viewStore.send(.animategrid)
                    viewStore.send(.unanimate) }
                Spacer()
                ThemedButton(text: "Random") { viewStore.send(.resetGridToRandom,animation: .interpolatingSpring(stiffness: 100, damping: 30))
                    viewStore.send(.animategrid)
                    viewStore.send(.unanimate) }
                Spacer()
            }
